...
3. 具备 Text-Expander 功能(可在偏好设置Web页面 http://127.0.0.1:62718 中添加/删除)。 用户可以定义自己常用的词组，比如 `{"yem":"you expand me"}`，那么当输入 `yem` 时会显示 `you expand me`。
...
> **⚠️ Attention:** macOS 14 以上系统用户需要在 Input source 中手动添加 YingHan 输入法：
...
1. `open YingHan.xcworkspace` 使用 Xcode 打开 `YingHan.xcworkspace` 工程，注意不是打开 `YingHan.xcodeproj`。
2. `command + b` 构建.
...
2. 没有 log 输出时，可以查看崩溃日志，位置可通过 `ls -l ~/Library/Logs/DiagnosticReports/ | grep YingHan` 命令来查找。
3. 深思熟虑。
...
1. **英文词库数据库**: `~/Library/Application Support/YingHan/words_with_frequency_and_translation_and_ipa.sqlite3`
...
3. **自定义替换数据库**: `~/Library/Application Support/YingHan/substitutions.sqlite3`
   ...
   - 可在偏好设置页面 (http://127.0.0.1:62718) 中添加/删除
   - 安装和更新时保留（不会被覆盖）
